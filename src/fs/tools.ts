...
export const FILESYSTEM_TOOL_SUMMARIES: ToolSummary[] = [
...
export function registerFilesystemTools(server: McpServer, allowedDirectories: string[]): ToolSummary[] {
  if (allowedDirectories.length === 0) return []
  const context = createFilesystemContext(allowedDirectories)

  const readTextFileHandler = async (args: z.infer<typeof readTextFileSchema>) => {
  ...
  const readMediaHandler = async ({ path: requestedPath }: z.infer<typeof readMediaFileSchema>) => {
    ...
    const content =
      type === 'image'
        ? [{ type: 'image' as const, data: await readFileAsBase64(validPath), mimeType }]
        : type === 'audio'
          ? [{ type: 'audio' as const, data: await readFileAsBase64(validPath), mimeType }]
          : [{ type: 'blob' as const, data: await readFileAsBase64(validPath), mimeType }]
  ...
  server.registerTool(
    ...
    async ({ path: requestedPath, sortBy }) => {
      ...
      const detailed = await Promise.all(
        (await fs.readdir(validPath, { withFileTypes: true })).map(async (entry) => {
