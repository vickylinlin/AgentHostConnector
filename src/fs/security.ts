...
export function isPathWithinAllowedDirectories(absolutePath: string, allowedDirectories: string[]): boolean {
  if (!absolutePath || allowedDirectories.length === 0 || absolutePath.includes('\x00')) return false
  const normalizedPath = path.resolve(path.normalize(absolutePath))

  return allowedDirectories.some((dir) => {
    if (!dir || dir.includes('\x00')) return false
...
export async function resolveAllowedDirectories(directories: string[]): Promise<{ allowedDirectories: string[]; warnings: string[] }> {
  ...
  for (const directory of directories) {
    ...
    try {
      ...
      if (!allowedDirectories.includes(real)) allowedDirectories.push(real)
      if (real !== absolute && !allowedDirectories.includes(absolute)) allowedDirectories.push(absolute)
    } catch (error) {
...
function resolveRelativePath(relativePath: string, allowedDirectories: string[]): string {
  for (const allowedDir of allowedDirectories) {
    const candidate = path.resolve(allowedDir, relativePath)
    if (isPathWithinAllowedDirectories(candidate, allowedDirectories)) return candidate
  }
  return path.resolve(allowedDirectories[0], relativePath)
}

export function createFilesystemContext(allowedDirectories: string[]): FilesystemContext {
  return {
