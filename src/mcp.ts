...
function registerSkillCapabilities(server: McpServer, config: LoadedConfig, logger: Logger) {
...
export async function createMcpHost(config: LoadedConfig, allowedDirectories: string[], logger: Logger): Promise<McpHost> {
  const server = new McpServer(
    { name: 'agent-host-connector', version: '0.1.0' },
    {
      capabilities: {
      ...
      instructions:
        'This local connector serves Agent Skills as MCP resources. Read skill://index.json to discover skills, then read skill://<skill-name>/SKILL.md and referenced skill://<skill-name>/<relative-path> files on demand. Filesystem tools are available only for explicitly allowlisted directories.',
    },
  )
...
export function createBaseApp(host: string): Hono {
